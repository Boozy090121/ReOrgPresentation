...
import { ChevronDown, ChevronUp, UserCircle, Users, Clipboard, ClipboardCheck, AlertCircle, BarChart, Calendar, DollarSign, Home, Beaker, UserPlus, XCircle, Move, Save } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const clientTeams = [
    { manager: "Quality Manager 1", clients: ["Lilly", "Novo"] },
    { manager: "Quality Manager 2", clients: ["GSK", "BMS"] },
    { manager: "Quality Manager 3", clients: ["Merck", "AZ"] }
  ];
  
  // Personnel Panel Component
  const PersonnelPanel = () => {
    const availablePersonnel = personnel.filter(person => !person.assignedRole);
    const assignedPersonnel = personnel.filter(person => person.assignedRole);
    
    return (
      <div className="bg-white rounded-lg shadow-md p-4 mb-6">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-bold" style={{ color: colors.primary }}>Personnel Management</h3>
          <div className="flex space-x-2">
            ...
            >
              <UserPlus size={16} className="mr-1" /> Add Person
            </button>
            <button
              className="flex items-center px-3 py-1 rounded text-white text-sm"
